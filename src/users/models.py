...
class DynamicEmailConfiguration(models.Model):
    ...
    """
    email_name = models.CharField(unique=True, max_length=256, verbose_name = "email unique name")
    
    slug = models.SlugField(verbose_name = "slug", unique=True, default=slugify(email_name))
    ...
    def __str__(self):
        return f"{self.slug} - {self.from_email}"
